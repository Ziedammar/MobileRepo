...
export default function AdminProfile() {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView>
        <View style={[styles.headerBg, { backgroundColor: Colors.adminPrimary }]}>
          ...
          <View style={styles.roleBadge}>
            <Text style={styles.roleText}>ADMIN RESTAURANT</Text>
          </View>
        </View>

        <View style={styles.menuCard}>
          {[
            { icon: 'storefront-outline' as const, label: 'Mon restaurant', onPress: () => {} },
            { icon: 'bicycle-outline' as const, label: 'Gérer les livreurs', onPress: () => {} },
            ...
            <React.Fragment key={item.label}>
              <TouchableOpacity style={styles.menuRow} onPress={item.onPress}>
                <View style={styles.menuIcon}>
                  <Ionicons name={item.icon} size={20} color={Colors.adminPrimary} />
                </View>
...
const styles = StyleSheet.create({
  ...
  menuIcon: { width: 40, height: 40, borderRadius: BorderRadius.md, backgroundColor: Colors.adminPrimaryLight, alignItems: 'center', justifyContent: 'center' },
